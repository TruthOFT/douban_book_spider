...
class BookSpider(RedisSpider):
    ...
    def __retry(self, res: HtmlResponse, **kwargs):
        self.logger.info(f"### retry {res.url} ###")
        yield Request(url=res.url, callback=self.parse)

    def parse(self, response: HtmlResponse, **kwargs):
        if response.status == 302:
            self.logger.warning(f"### {response.url} meet 302 ###")
            self.logger.warning(f"### redirect {response.headers.get('Location')} ###")
            yield Request(url=response.url, callback=self.__retry)
        else:
            self.logger.info(f"### res.url {response.url}")
            item = kwargs["item"]
